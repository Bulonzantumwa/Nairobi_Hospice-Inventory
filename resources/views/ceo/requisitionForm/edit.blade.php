        ...
        <main>
            <div class="mt-2 w-3/4 justify-center mx-auto">
                <div class="flex items-center justify-end px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
                    <a href="{{ route('ceo.requisitionForm.index') }}" class="px-4 py-2 text-sm text-white rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring focus:ring-primary focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark">
                        View Requisitions
                    </a>
                </div>
                @if($requisitionItems->count() > 0)
                    ...
                    <form method="POST" action="{{ route('ceo.requisitionForm.forward', $requisitionForm) }}" class="shadow-lg rounded-md bg-gray-100 py-2">
                        @method('PUT')
                        @csrf
                        <div class="mb-6">
                            <label for="ceoRemarks" class="focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light w-3/4 justify-center mx-auto block mb-2 text-sm font-medium text-gray-900 dark:text-white">Remarks</label>
                            <textarea rows="5" id="ceoRemarks" name="ceoRemarks" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light w-3/4 justify-center mx-auto" placeholder="Remarks" required></textarea>
                        </div>
                        @error('ceoRemarks')<span class="text-red-400 text-sm">{{ $message }}</span>@enderror
                        <div class="flex">
                            <button type="submit" name="status" value="3" class="flex items-center px-4 py-2 text-sm text-white rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring focus:ring-primary focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark ml-5">Approve</button>
                            <button type="submit" name="status" value="-3" class="flex items-center px-4 py-2 text-sm text-white rounded-md bg-red-500 hover:bg-red-500-dark focus:outline-none focus:ring focus:ring-primary focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark ml-5">Reject</button>
                        </div>
                    </form>
                @else
