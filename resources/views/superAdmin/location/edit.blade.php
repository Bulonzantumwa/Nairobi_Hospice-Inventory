        ...
        <main>
            <div class="mt-2 w-3/4 justify-center mx-auto">
                <div class="flex items-center justify-end px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
                    <a href="{{ route('superAdmin.utility.index') }}" class="px-4 py-2 text-sm text-white rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring focus:ring-primary focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark">
                        View utilities
                    </a>
                </div>
                <form method="POST" action="{{ route('superAdmin.utility.update', $utility) }}" class="shadow-lg rounded-md bg-gray-100 py-2">
                    @method('PUT')
                    @csrf
                    <div class="mb-6">
